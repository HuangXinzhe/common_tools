...
class DataToNeo4j(object):
    ...
    def __init__(self):
        """建立连接"""
        # link = Graph("http://localhost:7687", username="neo4j", password="neo4j")
        link = Graph("bolt://localhost:7687", auth=("my_neo4j", "1"))
        self.graph = link
        # self.graph = NodeMatcher(link)
        # 定义label
        self.buy = 'buy'
        self.sell = 'sell'
        self.graph.delete_all()  # 构建之前先清理数据库中已有的数据
        self.matcher = NodeMatcher(link)  # 匹配关系时使用
