"""readme
v1.1 修正早停功能模块，并增加连续早停或累计早停功能选择。
v1.0 实现CRF模型功能，并测试通过模型训练的有效性
"""
import numpy as np
...
class LinerCRF(tf.keras.Model):
    def __init__(self, tag_size):
        ...
        # self.Hidden = [layers.GRU(64, return_sequences=True) for _ in range(1)]
    ...
    def call(self, inputs):
        ...
        #     hidden = Hidden(hidden, mask=mask)
        return self.Output(hidden, mask)

    def train(self, train_data, valid_data=None
              , epochs=10, gard_theta=1
              , patience=None, min_delta=1e-3, early_stop_mode='max'
              , checkpoint_dir='./checkpoints/'
              , init_best_accuracy=0
              , verbose=0):
        """
        early_stop_mode = 'max' or 'min'
        """
        #
        ...
        for epoch in range(epochs):
            ...
            if patience is not None and len(metrics_pre_epoch['val_accuracy']) > 1:
                ...
                # else:
                #     """
                #     开启本段代码：即连续递增或递减patience次后，提前停止训练
                #     关闭本段代码：即累计递增或递减patience次后，提前停止训练
                #     """
                #     self.patience = 0
